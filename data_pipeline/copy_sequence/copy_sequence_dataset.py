...
from constants import SENTENCE_KEY, SENTENCE_LEN_KEY
...
def numericalize(raw_text_iter, vocab: CopySequenceVocab, tokenizer):
    ...
    return filter(lambda t: t.numel() > 2, data)


def build_vocab(dataset, tokenizer, special_words):
...
class CopySequenceDataset(Dataset):
    def __init__(self, vocab, tokenizer, max_sen_len: bool = None):
        ...
        sentences = numericalize(test_iter, vocab, self.tokenizer)
...
if __name__ == "__main__":
    ...
    train_iter = WikiText2(split='test')
    tokenizer = get_tokenizer('basic_english')
    special_words = [PAD, EOS, UNK]

    vocab = build_vocab(train_iter, tokenizer, special_words)
    print(vocab.token_vocab.keys())
