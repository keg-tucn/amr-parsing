...
def get_cache_paths():
  ...
  cache_files = [TOKENS_CACHE_FILE, CONCEPTS_CACHE_FILE, RELATIONS_CACHE_FILE, LEMMAS_CACHE_FILE]
...
def read_cached_vocabs():
  ...
  for cache_path in cache_paths:
    if os.path.isfile(cache_path):
      ...
      return None
  return vocabs[0], vocabs[1], vocabs[2], vocabs[3]
...
def cache_vocabs(token_vocab: Dict[str, int],
  ...
  if not os.path.exists(cache_dir):
    ...
    return None
  cache_paths = get_cache_paths()
  vocabs = [token_vocab, concept_vocab, relation_vocab, lemmas_vocab]
...
def build_vocabs(paths: List[str],
  ...
  # Cache the vocabs.
  cache_vocabs(token_vocab, concept_vocab, relation_vocab, lemmas_vocab)
  return token_vocab, concept_vocab, relation_vocab, lemmas_vocab

# TODO bulid lemma vocab here
...
class Vocabs():
  ...
  def __init__(self,
    ...
    token_vocab, concept_vocab, relation_vocab, lemmas_vocab = build_vocabs(
