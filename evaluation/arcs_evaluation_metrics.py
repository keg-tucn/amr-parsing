...
def calc_edges_scores(gold_mat: torch.tensor, predictions: torch.tensor, inputs_lengths: torch.tensor):
    ...
    Returns:
        F_score, precision, recall and accuracy
        between unpadded gold data and edge actual predictions
    '''
    index = 0
    f_score = 0
    accuracy = 0
...
def calc_labels_scores(gold_mat: torch.Tensor,
    ...
    for example in range(inputs_lengths.shape[0]):
        ...
        gold_edges = gold_mat[index][:sentence_len, :sentence_len]
        pred_edges = predictions[index][:sentence_len, :sentence_len]
